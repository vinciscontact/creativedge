...
// original .jpg files; we swap to the optimized .webp siblings generated by
// scripts/optimize-images.mjs.
const BASE = "/images/portfolio projects";
const u = (folder, file) => encodeURI(asset(`${BASE}/${folder}/${file.replace(/\.(jpe?g|png)$/i, ".webp")}`));
...
const IMAGES = [
    ...
    // Kept distinct from the featured Maiyson Therys case study shown right
    // below the hero, so the same artwork never appears twice back-to-back.
    { src: page("SM-DESIGN", "09", 3), label: "Social Media" },
];
...
function FlipCard({ src, label, index, scatter, phaseMv, cw, ch, morph, rotate, parallax }) {
    ...
    // Snappy but still soft — higher stiffness keeps the cards glued to the
    // scrollbar instead of rubber-banding behind it on the first wheel ticks.
    const cfg = { stiffness: 190, damping: 26, mass: 0.45 };
            ...
            >
                ...
                >
                    <div className="text-center">
                        <p className="text-[7px] font-black text-accent uppercase tracking-[0.2em] mb-1">CreativzEdge</p>
...
export default function ScrollMorphHero() {
    ...
    // Tall outer wrapper provides the scroll distance; the inner container is
    // sticky-pinned to the viewport while the morph plays out.
    ...
    // --- Native scroll drives everything ---
    // No wheel/touch hijacking: the page scrolls normally and we read the
    // section's scroll progress (0 → 1 across the tall wrapper). This is what
    // keeps the hero silky — browser momentum stays intact and the user can
    // never get "stuck" inside the animation.
    const { scrollYProgress } = useScroll({
        target: wrapperRef,
        offset: ["start start", "end end"],
    });

    // Morph: 0 (circle) -> 1 (bottom arc) over the first half of the section.
    // Shuffle rotation plays out over the remaining scroll distance.
    // Reduced motion: both stay at 0 — the assembled circle simply scrolls past.
    const morph = useTransform(scrollYProgress, [0, 0.5], reducedMotion ? [0, 0] : [0, 1]);
    const scrollRotate = useTransform(scrollYProgress, [0.5, 1], reducedMotion ? [0, 0] : [0, 360]);

    // If the visitor scrolls before the timed intro finishes, don't make them
    // wait — jump straight to the assembled circle so the morph answers their
    // scroll immediately. (This was the "stuck on the first scrolls" feeling:
    // during the 2.5s intro the cards ignored scroll input entirely.)
    useEffect(() => {
        if (reducedMotion) return;
        const unsubscribe = scrollYProgress.on("change", (v) => {
            if (v > 0.01) setIntroPhase((p) => (p === "circle" ? p : "circle"));
        });
        return () => unsubscribe();
    ...
    useEffect(() => {
        ...
        if (reducedMotion) return;
        const timer1 = setTimeout(() => setIntroPhase("line"), 350);
    ...
    return (
        <section
            ref={wrapperRef}
            className={`relative w-full ${reducedMotion ? "h-screen" : "h-[250vh]"}`}
        >
            <div ref={containerRef} className="sticky top-0 h-screen w-full bg-background overflow-hidden">
                <div className="flex h-full w-full flex-col items-center justify-center" style={{ perspective: "1000px" }}>
                    ...
                    {/* z-20 keeps the headline in front of the orbiting cards
                        (pointer-events-none, so card hovers still work through it) */}
                    <motion.div
                        style={{ opacity: introFade }}
                        className="absolute z-20 flex flex-col items-center justify-center text-center pointer-events-none px-6 w-full max-w-sm md:max-w-none top-[13%] -translate-y-0 md:top-1/2 md:-translate-y-1/2"
                    >
                        <motion.h1
                    ...
                    <motion.div
                        ...
                        className="absolute top-[12%] z-20 flex flex-col items-center justify-center text-center pointer-events-none px-4"
                    >
